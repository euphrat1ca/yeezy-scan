...
namespace windowsmanger
{
	public class FormAdmin : Form
	{
		...
		private void AddItem2listViewAdminEntrance(string ItemText)
		{
			...
			FormAdmin.dd method = new FormAdmin.dd(this.AddItem2listViewAdminEntrance);
		...
		private void btnSearchEngine_Click(object sender, EventArgs e)
		{
			try
			{
				...
				string[] linksFromSource = this.mainfrm.CurrentSite.GetLinksFromSource(sourceCode, "http://www.google.com/", "");
				for (int i = 0; i < linksFromSource.Length; i++)
				{
					string str3 = linksFromSource[i];
				...
				if (this.mainfrm.CurrentSite.GetHttpWebResponse(sURL, RequestType.GET).StatusCode == HttpStatusCode.OK)
				{
					...
					if (!string.IsNullOrEmpty(this.mainfrm.CurrentSite.FileExt))
					{
						...
						for (int i = 0; i < length; i++)
						{
							...
							string str2 = sURL + strArray[i] + this.mainfrm.CurrentSite.FileExt;
							if (this.mainfrm.CurrentSite.GetHttpWebResponse(str2, RequestType.GET).StatusCode == HttpStatusCode.OK)
							{
								this.AddItem2listViewAdminEntrance(str2);
								this.mainfrm.DisplayProgress("Checking: " + str2 + " OK!");
							}
							else
							{
								this.mainfrm.DisplayProgress("Checking: " + str2 + " NotFound!");
							}
						}
					}
				}
		...
		{
			ComponentResourceManager resources = new ComponentResourceManager(typeof(FormAdmin));
			...
			this.toolStripAdmin.Items.AddRange(new ToolStripItem[]
			{
			...
			this.toolStripAdmin.Location = new Point(0, 298);
			this.toolStripAdmin.Name = "toolStripAdmin";
			this.toolStripAdmin.Size = new Size(485, 25);
			...
			this.btnGetAdmin.Size = new Size(93, 22);
			this.btnGetAdmin.Text = "获取管理员页面";
			...
			this.btnSearchEngine.Size = new Size(147, 22);
			...
			this.listViewAdminEntrance.Columns.AddRange(new ColumnHeader[]
			{
				this.columnHeader4
			});
			...
			this.listViewAdminEntrance.Size = new Size(485, 298);
			...
			this.columnHeader4.Text = "后台管理入口";
			this.columnHeader4.Width = 710;
			base.AutoScaleDimensions = new SizeF(6f, 12f);
			base.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			base.ClientSize = new Size(485, 323);
